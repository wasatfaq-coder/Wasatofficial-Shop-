          ...
          >
            ...
            <div className="flex items-center justify-between pb-3 border-b border-[#BAC5D5]/40">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl neu-inset flex items-center justify-center text-[#5F6ED0] bg-[#E3E8EF]">
              ...
              <button
                onClick={onClose}
                className="w-9 h-9 rounded-full neu-button flex items-center justify-center text-[#5C6B80] hover:text-[#2D3A4E] transition-all cursor-pointer bg-[#E3E8EF]"
                aria-label="Закрыть"
              >
            ...
            <div className="neu-inset rounded-2xl p-3.5 border border-white/50 space-y-2 bg-[#E3E8EF] shadow-inner transition-all duration-200">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="text-[10px] font-extrabold text-[#5F6ED0] flex items-center gap-1.5 ml-auto">
                  <span className="text-[#5C6B80] font-medium">Прибавка к ПОГ:</span>
              ...
              <p className="text-[11px] text-[#5C6B80] leading-relaxed font-medium pl-0.5">
                {silhouetteData.fitDescription}
              </p>

              {/* Dynamic Micro-metrics bar inside the Inset banner */}
              <div className="grid grid-cols-3 gap-1.5 pt-1.5 border-t border-[#BAC5D5]/40 text-[10px]">
                <div className="neu-flat rounded-lg px-2 py-1 bg-[#E3E8EF] border border-white/60 text-center">
                  <span className="text-[#5C6B80] text-[9px] block">ПОГ изделия</span>
                  <span className="font-extrabold text-[#2D3A4E]">
                    {silhouetteData.halfChest} см
                  </span>
                </div>
                <div className="neu-flat rounded-lg px-2 py-1 bg-[#E3E8EF] border border-white/60 text-center">
                  <span className="text-[#5C6B80] text-[9px] block">Дроп (ОГ − ОТ)</span>
                  <span className="font-extrabold text-[#5F6ED0]">
                    {silhouetteData.drop > 0 ? `+${silhouetteData.drop}` : silhouetteData.drop} см
                  </span>
                </div>
                <div className="neu-flat rounded-lg px-2 py-1 bg-[#E3E8EF] border border-white/60 text-center">
                  <span className="text-[#5C6B80] text-[9px] block">Тип телосложения</span>
                  <span className="font-extrabold text-[#2D3A4E]">
            ...
            <div className="neu-inset rounded-2xl p-3.5 space-y-3 bg-[#E3E8EF] border border-white/50">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-[10px] font-bold text-[#5F6ED0] neu-button px-2.5 py-1 rounded-lg flex items-center gap-1 cursor-pointer bg-[#E3E8EF]"
                  >
                    <span>Таблица</span>
                    {showSizeTable ? (
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-[#5C6B80] hover:text-[#2D3A4E] neu-button px-2.5 py-1 rounded-lg cursor-pointer bg-[#E3E8EF]"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <div className="neu-flat rounded-xl p-2.5 text-center bg-[#E3E8EF] border border-white/70">
                  <span className="text-[9px] font-medium text-[#5C6B80] block">Верхняя одежда</span>
                ...
                <div className="neu-flat rounded-xl p-2.5 text-center bg-[#E3E8EF] border border-white/70">
                  <span className="text-[9px] font-medium text-[#5C6B80] block">Брюки / Джинсы</span>
                  <span className="text-xs font-black text-[#2D3A4E] block my-0.5">
                    {russianPattern.bottomSizeLabel}
                  </span>
                  <span className="text-[9px] text-[#5C6B80] block">Пояс: {waist} см</span>
                </div>

                <div className="neu-flat rounded-xl p-2.5 text-center bg-[#E3E8EF] border border-white/70">
                  <span className="text-[9px] font-medium text-[#5C6B80] block">Ростовка РФ</span>
                  <span className="text-xs font-black text-[#2D3A4E] block my-0.5">
                ...
                <div className="neu-flat rounded-xl p-2.5 text-center bg-[#E3E8EF] border border-white/70">
                  <span className="text-[9px] font-medium text-[#5C6B80] block">Полнота / ИМТ</span>
                  <span className="text-xs font-black text-[#2D3A4E] block my-0.5">
                    {russianPattern.fullnessGroup}-я полнота
                  </span>
                  <span className="text-[9px] text-[#5C6B80] block">
              ...
              <div className="flex items-center justify-between text-[11px] font-medium text-[#5C6B80] pt-1 border-t border-[#BAC5D5]/40 flex-wrap gap-1">
                <span>{russianPattern.recommendedFit}</span>
                <span className="text-[#5F6ED0] font-bold">Точность {confidence}%</span>
              </div>
              ...
              {showSizeTable && (
                <div className="neu-flat rounded-xl p-3 bg-[#E3E8EF] border border-white/80 space-y-2 mt-2 animate-in fade-in">
                  <div className="text-[11px] font-extrabold text-[#2D3A4E] flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-[#5F6ED0] font-bold">Стандарт РФ</span>
                  </div>
            ...
            >
              ...
              <span className="text-xs font-bold text-[#2D3A4E] flex items-center gap-1.5">
                <BookmarkCheck className="w-3.5 h-3.5 text-[#5F6ED0]" />
            ...
            <div className="flex gap-2.5 pt-1">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-3.5 neu-button rounded-2xl text-[#5C6B80] font-bold text-xs hover:text-[#2D3A4E] cursor-pointer active:scale-95 transition-transform bg-[#E3E8EF]"
              >
                Закрыть
              </button>
              <button
