      ...
      {isOpen && order && (
        ...
        <div className="flex items-start justify-between gap-3 border-b border-[#BAC5D5]/50 pb-3">
          <div className="flex items-start gap-3 min-w-0">
            <div className="w-10 h-10 rounded-2xl neu-button flex items-center justify-center text-[#5F6ED0] shrink-0 mt-0.5">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs font-mono font-black neu-inset px-2.5 py-0.5 rounded-lg text-[#5F6ED0] bg-[#E3E8EF] shrink-0">
                  № {order.id}
                </span>
              </div>
              <p className="text-xs text-[#5C6B80] font-medium leading-relaxed mt-1">
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-8 h-8 rounded-xl neu-button flex items-center justify-center text-[#5C6B80] hover:text-[#2D3A4E] cursor-pointer shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="neu-inset rounded-2xl p-3 bg-[#E3E8EF] grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
          <div className="flex items-center gap-2">
            <span className="text-[#5C6B80] font-bold shrink-0">Статус заказа:</span>
            <span className="neu-flat px-2.5 py-1 rounded-lg font-black text-[#5F6ED0]">
          ...
          <div className="flex items-center gap-2 text-[#5C6B80] min-w-0">
        ...
        <div className="neu-flat rounded-2xl p-4 sm:p-5 space-y-3 bg-[#E3E8EF] border border-white/60">
          <div className="flex items-center justify-between">
            <label className="text-xs font-black uppercase text-[#2D3A4E] tracking-wider flex items-center gap-1.5">
              <Package className="w-3.5 h-3.5 text-[#5F6ED0]" />
              Состав позиций в заказе ({items.length})
            </label>
            <button
              type="button"
              onClick={() => setIsAddingItem(!isAddingItem)}
              className="py-1.5 px-3 neu-button rounded-xl text-xs font-extrabold text-[#5F6ED0] flex items-center gap-1.5 hover:scale-105 transition-transform cursor-pointer whitespace-nowrap"
            >
          ...
          {isAddingItem && (
            <div className="neu-flat rounded-2xl p-3.5 bg-[#E3E8EF] border border-[#5F6ED0]/30 space-y-3 animate-in fade-in duration-150">
              <div className="flex items-center justify-between border-b border-[#BAC5D5]/40 pb-2">
                <span className="text-xs font-extrabold text-[#5F6ED0] flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="text-xs text-[#5C6B80] hover:text-[#2D3A4E] font-bold"
                >
                  Закрыть
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 text-xs">
                ...
                <div className="sm:col-span-6">
                  <label className="text-[11px] font-bold text-[#5C6B80] block mb-1">Товар</label>
                  <NeumorphicSelect
                ...
                <div className="sm:col-span-3">
                  <label className="text-[11px] font-bold text-[#5C6B80] block mb-1">Цвет</label>
                  <NeumorphicSelect
                    value={selectedColor}
                ...
                {/* Select Size */}
                <div className="sm:col-span-3">
                  <label className="text-[11px] font-bold text-[#5C6B80] block mb-1">Размер</label>
                  <NeumorphicSelect
                    value={selectedSize}
              ...
              <div className="flex items-center justify-between pt-1 flex-wrap gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-[#5C6B80]">Количество:</span>
                  <div className="flex items-center gap-1 neu-inset rounded-xl p-1 bg-[#E3E8EF]">
                    <button
                      type="button"
                      onClick={() => setAddQuantity(Math.max(1, addQuantity - 1))}
                      className="w-6 h-6 neu-button rounded-lg flex items-center justify-center text-xs font-bold text-[#5C6B80]"
                    >
                      -
                    </button>
          ...
          <div className="space-y-2.5 max-h-64 overflow-y-auto pr-1 no-scrollbar">
            {items.length === 0 ? (
              <div className="neu-inset rounded-2xl p-4 text-center text-xs text-[#5C6B80] bg-[#E3E8EF]">
                      ...
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1 space-y-1">
                          ...
                          <div className="flex items-center gap-2 flex-wrap text-xs">
                            ...
                            <span className="neu-flat px-2 py-0.5 rounded-lg text-[11px] font-black text-[#5F6ED0]">
                              {item.selectedSize}
                            </span>
                          </div>
                        </div>
                    ...
                    <div className="flex items-center justify-between border-t border-[#BAC5D5]/40 pt-2 flex-wrap gap-2">
                      ...
                      <div className="flex items-center gap-1 neu-inset rounded-xl p-1 bg-[#E3E8EF]">
                        <button
                          ...
                          className="w-7 h-7 rounded-lg neu-button flex items-center justify-center text-xs font-black text-[#5C6B80] hover:text-[#2D3A4E] cursor-pointer"
                          title="Уменьшить"
                        >
        ...
        <div className="neu-flat rounded-2xl p-4 sm:p-5 space-y-3 bg-[#E3E8EF] border border-white/60">
          <label className="text-xs font-black uppercase text-[#2D3A4E] tracking-wider flex items-center gap-1.5">
            <UserCheck className="w-3.5 h-3.5 text-[#5F6ED0]" />
          ...
          <input
            ...
            className="w-full px-3 py-2.5 neu-inset rounded-xl text-xs text-[#2D3A4E] placeholder:text-[#5C6B80] bg-[#E3E8EF] focus:outline-none"
          />
        </div>
          ...
          if (!isTK) {
            ...
            return (
              <div className="neu-flat rounded-2xl p-4 sm:p-5 space-y-2 bg-[#E3E8EF] border border-white/60">
                ...
                <div className="neu-inset rounded-xl p-3 bg-[#E3E8EF] text-[11px] text-[#5C6B80] leading-relaxed">
              ...
              <div className="flex items-center gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full pl-3 pr-8 py-2.5 neu-inset rounded-xl text-xs font-mono font-bold text-[#2D3A4E] placeholder:text-[#5C6B80] placeholder:font-sans bg-[#E3E8EF] focus:outline-none"
                  />
                  {trackingNumber && (
                    <button
                      type="button"
                      onClick={() => setTrackingNumber('')}
                      className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#5C6B80] hover:text-[#2D3A4E] cursor-pointer"
                ...
                <button
                  ...
                  className="py-2.5 px-3 neu-button rounded-xl text-xs font-bold text-[#5F6ED0] hover:scale-105 active:scale-95 transition-transform cursor-pointer shrink-0"
        ...
        <div className="neu-flat rounded-2xl p-4 space-y-2.5 bg-[#E3E8EF] border border-white/80">
          <div className="flex items-center justify-between text-xs font-bold text-[#5C6B80]">
          ...
          <div className="flex items-center justify-between text-sm font-extrabold text-[#2D3A4E]">
            ...
            <span className="text-[#5F6ED0] font-black text-base">
        ...
        <div className="pt-2 border-t border-[#BAC5D5]/50 flex flex-col-reverse sm:flex-row items-center justify-end gap-2.5">
          <button
            type="button"
            onClick={onClose}
            className="w-full sm:w-auto h-10 sm:h-11 px-4 sm:px-5 neu-button rounded-xl text-xs font-bold text-[#5C6B80] hover:text-[#2D3A4E] cursor-pointer text-center transition-all active:scale-95"
          >
            Отмена
          </button>
          <button
