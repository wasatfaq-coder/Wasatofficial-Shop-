    ...
    <div className="neu-inset rounded-2xl p-4 sm:p-5 bg-[#E3E8EF] border border-[#5F6ED0]/40 space-y-4 transition-all animate-in fade-in-50 duration-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-black uppercase tracking-wider text-[#5F6ED0]">
                Срез дня
              </span>
              {dayIndex !== undefined && totalDays !== undefined && (
                <span className="text-[10px] font-bold text-[#5C6B80] neu-inset px-2 py-0.2 rounded-md bg-[#E3E8EF]">
              ...
              {dayData.hasRealOrders && (
                <span className="text-[9px] font-black text-indigo-700 bg-indigo-100 px-2 py-0.5 rounded-full flex items-center gap-1">
            ...
            <h4 className="text-sm sm:text-base font-black text-[#2D3A4E] mt-0.5 flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-[#5F6ED0]">({dayData.weekday})</span>
            </h4>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          {onPrevDay && (
            <button
              ...
              disabled={!hasPrev}
              type="button"
              className="p-2 rounded-xl neu-button text-[#5C6B80] hover:text-[#2D3A4E] transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-1 text-[11px] font-bold active:scale-95"
          ...
          <button
            ...
            type="button"
            className="p-2 rounded-xl neu-button text-[#5C6B80] hover:text-[#2D3A4E] transition-all cursor-pointer ml-1 active:scale-95"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        ...
        <div className="neu-inset rounded-2xl p-3 bg-[#E3E8EF] space-y-1">
          <div className="flex items-center justify-between text-[#5C6B80]">
            <span className="text-[10px] font-bold uppercase tracking-wider">Выручка за сутки</span>
            <div className="w-5 h-5 rounded-lg neu-inset flex items-center justify-center text-[#5F6ED0] bg-[#E3E8EF]">
              <DollarSign className="w-3 h-3" />
            </div>
          </div>
          <p className="text-base sm:text-lg font-black text-[#5F6ED0] tracking-tight">
            {dayData.revenue.toLocaleString('ru-RU')} ₽
          </p>
          <div className="text-[10px] text-[#5C6B80] flex items-center justify-between">
        ...
        <div className="neu-inset rounded-2xl p-3 bg-[#E3E8EF] space-y-1">
          <div className="flex items-center justify-between text-[#5C6B80]">
            <span className="text-[10px] font-bold uppercase tracking-wider">Число заказов</span>
          ...
          <p className="text-base sm:text-lg font-black text-[#2D3A4E] tracking-tight">
            {dayData.orders} <span className="text-xs font-bold text-[#5C6B80]">шт.</span>
          </p>
          <div className="text-[10px] text-[#5C6B80] flex items-center justify-between">
        ...
        <div className="neu-inset rounded-2xl p-3 bg-[#E3E8EF] space-y-1">
          ...
          <p className="text-base sm:text-lg font-black text-[#2D3A4E] tracking-tight">
            {dayData.returns} <span className="text-xs font-bold text-[#5C6B80]">шт.</span>
          </p>
          <div className="text-[10px] text-[#5C6B80]">
            {dayData.returns === 0 ? (
      ...
      <div className="space-y-2.5 pt-1">
        <div className="flex items-center justify-between text-xs font-bold text-[#5C6B80]">
          ...
          <span className="text-[10px] neu-inset px-2.5 py-1 rounded-lg bg-[#E3E8EF] text-[#2D3A4E] font-extrabold">
        ...
        {dayData.realOrdersList.length > 0 ? (
              ...
              return (
                ...
                >
                  <div className="space-y-1.5 min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-mono font-black text-[#2D3A4E] text-[12px] group-hover:text-[#5F6ED0] transition-colors flex items-center gap-0.5">
                        <Hash className="w-3 h-3 text-[#5C6B80]" />
                        {ord.id}
                      </span>
                      <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded-full ${statusCfg.color}`}>
                        {statusCfg.label}
                      </span>
                      {ord.isAdjusted && (
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-[#5C6B80] flex-wrap">
                      {ord.customerName && (
                        <span className="flex items-center gap-1 font-medium text-[#2D3A4E]">
                          <User className="w-3 h-3 text-[#5C6B80]" />
                          {ord.customerName}
                        </span>
                      )}
                      {ord.customerPhone && (
                        <span className="flex items-center gap-1">
                          <Phone className="w-3 h-3 text-[#5C6B80]" />
                          {ord.customerPhone}
                        </span>
                      )}
                      <span>
                    ...
                    {ord.items && ord.items.length > 0 && (
                      <p className="text-[10px] text-[#5C6B80] truncate bg-white/40 px-2 py-0.5 rounded-md">
                  ...
                  <div className="text-right shrink-0 space-y-1">
                    <p className="text-sm font-black text-[#5F6ED0] tabular-nums">
                      {ord.totalPrice.toLocaleString('ru-RU')} ₽
                    </p>
                    <span className="text-[10px] text-[#5C6B80] block">
                      {ord.paymentMethod || 'Карта онлайн'}
                    </span>
                    <span className="text-[9px] font-bold text-[#5F6ED0] group-hover:underline flex items-center justify-end gap-0.5">
          ...
          <div className="neu-inset rounded-2xl p-3.5 text-center text-xs text-[#5C6B80] bg-[#E3E8EF] space-y-1">
