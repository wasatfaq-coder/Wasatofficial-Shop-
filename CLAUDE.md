...
- Чат: сообщения с `threadId` (uid покупателя; у гостя — анонимный uid из отдельного
  ...
  Бота и автоответов нет: покупатель пишет только `sender: 'user'` (правила), отвечают сотрудники; приветствие
  рисует `SupportChatModal` (в базу не пишется), статус отправки — настоящий (`pendingChatIds`/`failedChatMessages`).
  Админка: один список диалогов `AdminSupportInbox` → `AdminSupportChatTab`; статус и приоритет диалога —
  `support_threads/{threadId}` (только администратор). Поля ввода текста в админке — `resize-y`, как в FAQ.
